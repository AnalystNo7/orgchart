...
import { AI_ANSWER_MARKER, AI_KB_PROMPT_BUDGET_BYTES } from "./limits";
...
export function buildTechnicalPrompt(): string {
  return `## Формат ответа (ОБЯЗАТЕЛЬНО)
Всё, что ты пишешь до итогового ответа — план, рассуждения, промежуточные
заметки, — пользователю не показывается. Итоговый ответ (после всех вызовов
инструментов) начинай с ОТДЕЛЬНОЙ строки:

${AI_ANSWER_MARKER}

Эта строка обязательна, пишется ровно один раз за ответ и только перед итогом.
Не используй её внутри рассуждений и не пиши после неё ничего, кроме самого
ответа пользователю.
...
2. НЕ выводи в ответ планирование, перебор вариантов и сомнения. Перед действием — одна короткая фраза статуса, после — результаты и выводы. Это относится и к тексту между вызовами инструментов. Отсутствие нужных данных при этом называй явно, одной фразой.
3. Рассуждай кратко и на русском языке.

## Данные и их отсутствие (ОБЯЗАТЕЛЬНО)
1. Любое число в ответе (стоимость, FTE, проценты, суммы) берётся из результата инструмента в этом диалоге. Числа, которых нет в результатах, не выводи — ни как оценку, ни как пример.
2. Если нужного разреза нет в данных (например, нет стоимости по сотруднику или фактической загрузки), напиши «в данных нет X» и предложи ближайший доступный разрез (например, стоимость по подразделениям или покрытие договорами).
3. Имена людей и подразделений пиши ровно так, как они пришли из инструмента, включая обезличенные вида «сотрудник12» или номера. Не придумывай и не «восстанавливай» ФИО, должности и подразделения.
4. Агрегаты подразделения (стоимость на FTE, покрытие) не приписывай отдельным людям. Цифры по людям — только из get_employee_economics.
5. Единицы измерения бери из поля _units результата; не пересчитывай ₽/час в год сам — годовые суммы уже посчитаны инструментом.
...
4. Стоимость в год и покрытие договорами по сотрудникам — get_employee_economics (постранично, не более одного вызова за шаг; для «убыточных» — sortBy=costUncovered); по подразделениям — get_unit_economics; точный P&L — calculate_pnl.
