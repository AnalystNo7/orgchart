...
- UI: **Tailwind 4** + shadcn/ui (Radix, lucide), **@xyflow/react 12** + **dagre** (граф-канвасы), **zustand 5**, **@tanstack/react-table 8**, **react-hook-form 7** + **zod 4**, react-markdown + `@tailwindcss/typography` (только чат ассистента, класс `.chat-prose` в globals.css)
...
| AI-ассистент | `src/lib/ai/` (11 файлов) | `orchestrator.runChat` (streamText — текст стримится дельтами, ≤30 шагов tool-loop; SSE-событие `meta` — бюджет и номер шага для живого статуса; finish ≠ stop оформляется как честный обрыв через onError; chunk-обрыв (провайдер замолчал) отличается от таймаута по тишине; повторы при retryable-ошибках провайдера (429 лимит/слот шлюза, 529/503, сеть; `classifyProviderError`, Retry-After учитывается) — настраиваются в пресете, SDK `maxRetries: 0`, повтор только для «раннего» прогона (≤1 шага, ≤`AI_RETRY_MAX_SHOWN_CHARS` показанных символов, только read-only инструменты) с пометкой в чате; очередь к провайдеру `run-queue.ts` (семафор на процесс, позиция видна в UI); отмена клиентом (`req.signal`/`cancel()` → `AbortSignal` в `runChat`) прерывает очередь, паузы и стрим, освобождает слот сразу и сохраняет частичный ответ через `onAbort` без записи в закрытый поток; имена инструментов в тексте ответа заменяются русскими метками (`src/lib/ai/tool-labels.ts` — общий словарь сервера и чипов UI); рассуждения модели идут в тексте ответа как есть, скрывать их нельзя (шлюз Gonka/MiniMax не присылает закрывающий `</think>`, граница рассуждение/ответ не восстанавливается) — рассуждения скрываются на сервере потоковым фильтром `think-filter.ts` (читаем `fullStream`, а не `textStream`): граница «черновик/ответ» — строка-маркер `AI_ANSWER_MARKER` («### Ответ», её требует промпт), либо закрывающий `</think>`, либо структурно (текст шага, закончившегося вызовом инструментов, отбрасывается); если ни одного признака нет на финальном шаге — черновик показывается целиком (страховка, ответ не теряется), лог `[AI_REASONING]` с границей и объёмом; `reasoning-delta` от провайдера тоже уходит в скрытое; в UI фаза «Модель рассуждает…»; в БД и историю модели черновик не попадает. Старые беседы с закрытыми парами `<think>…</think>` разбираются при отрисовке (`think-split.ts`, `<details>`), незакрытый тег — маркером «💭 Размышляю…» (`replaceThinkTags` в `ChatMessage.tsx`), стиль «статус → результат» задан в технической части промпта; клон/what-if сценарий получает имя «Название (из: Родитель)» в точке клонирования, для старых связь показывает подстрочник из createdFrom); **tools.ts — 34 инструмента** (в т.ч. `get_employee_economics` — стоимость в год и покрытие доходными договорами по сотрудникам, единственный источник денежных цифр на уровне человека; `get_unit_economics` отдаёт ₽/год по рабочим часам года и `_units`), `wrapExecute` — единая точка: замер, кэш read-only вызовов за прогон, предохранитель бюджета контекста, `capToolResult`; `tool-executor.ts` — единый диспетчер, включая `run_whatif_scenario`; `local-query.ts` — ответы без LLM (бенчмарки/отклонения/поиск KB); во внешнюю модель сообщение уходит только по осознанному сигналу (`llm-gate.ts`): тумблер «AI» у поля ввода (флаг `useLlm` в запросе, состояние в localStorage, по умолчанию выключен), кнопка «Отправить в AI» и пять AI-чипов под заглушкой локального поиска (`done.needsLlm` → `AiMessage.askAi`, рендер только под последним сообщением), быстрые действия (`QuickActions.tsx`) — два набора по тумблеру: выключен → локальные чипы «Бенчмарки для ИТ-интеграторов», «Что у нас не в норме», «Оргструктура/Финансы/HR: нормы ИТ» (тексты подобраны под ключевые слова `local-query.ts`; детектор фильтрует по категории, не по метрике) и до двух чипов «БЗ: <документ>» по последним загруженным документам (`GET /api/knowledge`), включён → пять AI-промптов; после каждого ответа при выключенном тумблере над полем ввода снова показываются локальные чипы (блок «Что ещё посмотреть:» с крестиком), при включении тумблера посреди диалога — AI-чипы («AI включён. Что исследовать:»); «Сравнить» на gap-анализе всегда с флагом; префикс `!ai` в тексте сохранён для совместимости; `system-prompt.ts` — промпт из двух половин: методическая (редактируется в `/admin/settings/prompt`, хранится в `AiPromptSetting`; нет строки = дефолт из кода) + техническая (маркировка 【OSINT】/【KB】/【LLM】, порядок работы с инструментами, раздел «Данные и их отсутствие»: числа только из инструментов, имена ровно как в данных, нет разреза → сказать и предложить доступный; раздел «Язык ответа»: только русский, латиница лишь для терминов — только в коде); язык дополнительно страхуется `language.ts`: служебный напоминатель в копии последнего хода пользователя (в чат и БД не попадает) и автопочинка — при иероглифах/чужих письменностях в готовом ответе один вызов модели без инструментов «перепиши по-русски», результат принимается только без чужих символов и с сохранёнными числами, уходит событием `replace`, лог `[AI_LANG]` |
| Бюджеты AI-прогона | `src/lib/ai/limits.ts`, `tool-result-limit.ts`, пресет LLM | Все лимиты per-preset (пусто = дефолт из limits.ts): total `timeoutSec` ≤3600 / шаг `stepTimeoutSec` (600 с) / тишина стрима `chunkTimeoutSec` (120 с) / шагов `maxSteps` (30) / бюджет контекста `runContextBudgetBytes` (120 КБ) / `toolResultMaxBytes` (60 КБ на результат) / повторов `maxRetries` (3) × пауза `retryDelaySec` (20 с) / одновременных прогонов `maxConcurrentRuns` (1) / ожидание в очереди `queueTimeoutSec` (180 с); подрезка под `maxDuration` (300, литерал в `/api/ai/chat`) — только при `process.env.VERCEL`. Логи: `[AI_RUN]`, `[AI_QUEUE]`, `[AI_STEP_START]`, `[AI_TOOL]`, `[AI_STEP]`, `[AI_DONE]`, `[AI_BUDGET]`, `[AI_RETRY]`, `[AI_RETRY_SKIP]`, `[AI_ABORT]`, `[AI_REASONING]`, `[AI_RAW]` (по `AI_DEBUG_RAW_CHUNKS`), `[AI_CHAT_ERROR]` |
...
| Undo/Redo | `src/lib/action-logger.ts`, `action-labels.ts` | 18 типов действий через `ActionLog` (payload/undoPayload; в т.ч. `bulk_update_department_type` — массовая смена типа одним действием), API `/api/actions/*`, хоткеи в OrgChart. Стек **общий на сценарий** (правки справочников откатываются кнопкой на дашборде) — поэтому стор показывает строку «Отменено/Повторено: <метка>» рядом с кнопками (`undoRedoNotice`). Отмена `create_department`/`add_parent` отклоняется (409, `UndoRefusedError`), если под подразделением появились нелогированные дети/сотрудники — иначе дети становились новыми корнями. AI-инструменты, импорт и клонирование журнал **не пишут** |
