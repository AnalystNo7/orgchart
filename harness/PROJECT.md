...
| AI-ассистент | `src/lib/ai/` (10 файлов) | `orchestrator.runChat` (generateText, ≤10 шагов tool-loop); **tools.ts — 33 инструмента**, `wrapExecute` — единая точка: замер, кэш read-only вызовов за прогон, предохранитель бюджета контекста, `capToolResult`; `tool-executor.ts` — единый диспетчер, включая `run_whatif_scenario`; `local-query.ts` — ответы без LLM (бенчмарки/отклонения/поиск KB); `system-prompt.ts` — промпт из двух половин: методическая (редактируется в `/admin/settings/prompt`, хранится в `AiPromptSetting`; нет строки = дефолт из кода) + техническая (маркировка 【OSINT】/【KB】/【LLM】, порядок работы с инструментами — только в коде) |
