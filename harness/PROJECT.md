...
> Карта описывает ветку **dev-mvp3** (основная ветка разработки). Составлена по коду 2026-08-19.
...
| AI-ассистент | `src/lib/ai/` (10 файлов) | `orchestrator.runChat` (streamText — текст стримится дельтами, ≤30 шагов tool-loop); **tools.ts — 33 инструмента**, `wrapExecute` — единая точка: замер, кэш read-only вызовов за прогон, предохранитель бюджета контекста, `capToolResult`; `tool-executor.ts` — единый диспетчер, включая `run_whatif_scenario`; `local-query.ts` — ответы без LLM (бенчмарки/отклонения/поиск KB); `system-prompt.ts` — промпт из двух половин: методическая (редактируется в `/admin/settings/prompt`, хранится в `AiPromptSetting`; нет строки = дефолт из кода) + техническая (маркировка 【OSINT】/【KB】/【LLM】, порядок работы с инструментами — только в коде) |
| Бюджеты AI-прогона | `src/lib/ai/limits.ts`, `tool-result-limit.ts` | Тройной таймаут: total (пресет `timeoutSec` ≤3600, без пресета 30 мин) / `AI_STEP_TIMEOUT_MS` 10 мин на шаг / `AI_CHUNK_TIMEOUT_MS` 2 мин тишины стрима; подрезка под `maxDuration` (300, литерал в `/api/ai/chat` — Next читает статически) — только при `process.env.VERCEL`. `AI_MAX_STEPS` (30), `AI_RUN_CONTEXT_BUDGET_BYTES` (120 КБ на прогон), `DEFAULT_TOOL_RESULT_MAX_BYTES` (60 КБ на один результат). Логи: `[AI_RUN]`, `[AI_STEP_START]`, `[AI_TOOL]`, `[AI_STEP]`, `[AI_DONE]`, `[AI_BUDGET]`, `[AI_CHAT_ERROR]` |
...
- **P&L**: не учитывает «дженерики»/вакантные позиции (`docs/LIMITATIONS.md`); планируемые договоры без даты начала считаются с overlap=1 (пометка REV-007 в коде).
