...
| AI-ассистент | `src/lib/ai/` (10 файлов) | `orchestrator.runChat` (generateText, ≤10 шагов tool-loop); **tools.ts — 33 инструмента**, `wrapExecute` — единая точка: замер, кэш read-only вызовов за прогон, предохранитель бюджета контекста, `capToolResult`; `tool-executor.ts` — единый диспетчер, включая `run_whatif_scenario`; `local-query.ts` — ответы без LLM (бенчмарки/отклонения/поиск KB); `system-prompt.ts` — русский промпт с маркировкой источников 【OSINT】/【KB】/【LLM】 |
| Бюджеты AI-прогона | `src/lib/ai/limits.ts`, `tool-result-limit.ts` | `AI_ROUTE_MAX_DURATION_SEC` (300, дублирует литерал `maxDuration` в `/api/ai/chat` — Next читает его статически), `AI_LOOP_SAFETY_MS` (20 с запаса, чтобы SSE-ошибка успела уйти), `AI_RUN_CONTEXT_BUDGET_BYTES` (120 КБ на прогон), `DEFAULT_TOOL_RESULT_MAX_BYTES` (60 КБ на один результат). Логи прогона: `[AI_TOOL]`, `[AI_STEP]`, `[AI_DONE]`, `[AI_BUDGET]`, `[AI_CHAT_ERROR]` |
