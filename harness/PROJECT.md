...
| AI-ассистент | `src/lib/ai/` (10 файлов) | `orchestrator.runChat` (streamText — текст стримится дельтами, ≤30 шагов tool-loop; SSE-событие `meta` — бюджет и номер шага для живого статуса; finish ≠ stop оформляется как честный обрыв через onError; chunk-обрыв (провайдер замолчал) отличается от таймаута по тишине; 429 «too many concurrent requests» — авто-повтор 3×20 с, пока ничего не показано; имена инструментов в тексте ответа заменяются русскими метками (`src/lib/ai/tool-labels.ts` — общий словарь сервера и чипов UI); размышления модели `<think>…</think>` вырезаются из потока на сервере (`src/lib/ai/think-filter.ts`, стоит перед фильтром имён), уходят SSE-событием `reasoning` и полем `reasoning` сообщения ассистента в `AiConversation.messages`, в UI — свёрнутый блок «Ход рассуждений модели»; старые беседы с `<think>` в `content` разбираются при отрисовке (`splitLegacyThink`), стиль «статус → результат» задан в технической части промпта; клон/what-if сценарий получает имя «Название (из: Родитель)» в точке клонирования, для старых связь показывает подстрочник из createdFrom); **tools.ts — 33 инструмента**, `wrapExecute` — единая точка: замер, кэш read-only вызовов за прогон, предохранитель бюджета контекста, `capToolResult`; `tool-executor.ts` — единый диспетчер, включая `run_whatif_scenario`; `local-query.ts` — ответы без LLM (бенчмарки/отклонения/поиск KB); `system-prompt.ts` — промпт из двух половин: методическая (редактируется в `/admin/settings/prompt`, хранится в `AiPromptSetting`; нет строки = дефолт из кода) + техническая (маркировка 【OSINT】/【KB】/【LLM】, порядок работы с инструментами — только в коде) |
...
| Undo/Redo | `src/lib/action-logger.ts` | 18 типов действий через `ActionLog` (payload/undoPayload; в т.ч. `bulk_update_department_type` — массовая смена типа одним действием), API `/api/actions/*`, хоткеи в OrgChart |
